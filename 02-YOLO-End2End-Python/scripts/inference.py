#!/usr/bin/env python3
"""
Main entry point for YOLOv5-SU inference
This script allows running inference with different model precisions.
"""
import argparse
import os
import sys
...
def preprocess(image, img_size=640):
    ...
    # Return both tensor and info for coordinate mapping
    info = (ratio, start_x, start_y, w, h)
...
class YOLOv5SU:
    ...
    def postprocess(self, predictions, info):
...
def main():
    parser = argparse.ArgumentParser(description='YOLOv5-SU Inference')
    ...
    parser.add_argument('--precision', type=str, choices=['fp32', 'fp16', 'int8'], default=None,
                       help='Model precision to use (optional, inferred from model name)')
    ...
    if len(parts) >= 2:
        ...
        # Use --precision argument if provided, otherwise infer from model name
        if args.precision:
        ...
        model_arch = "unknown"
        precision = args.precision or "unknown"
