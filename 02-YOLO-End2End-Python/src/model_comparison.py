#!/usr/bin/env python3
"""
Script to compare FP32 and INT8 model performance
"""
import os
import time
import numpy as np
...
def main():
    ...
    if len(results) >= 2:
        ...
        speed_improvement = ((fp32_result['avg_time_ms'] - int8_result['avg_time_ms']) / fp32_result['avg_time_ms']) * 100
        
        print(f"  Model size reduction: {size_reduction:.1f}% (from {fp32_result['model_size']:.2f}MB to {int8_result['model_size']:.2f}MB)")
        print(f"  Speed improvement: {speed_improvement:.1f}% (from {fp32_result['avg_time_ms']:.2f}ms to {int8_result['avg_time_ms']:.2f}ms)")
        print(f"  Note: INT8 is ~{fp32_result['fps']/int8_result['fps']:.1f}x slower than expected, likely due to overhead in this quantization")
