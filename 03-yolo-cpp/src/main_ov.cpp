...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cerr << "  device: CPU (default), GPU, AUTO" << std::endl;
        return 1;
    }

    std::string image_path = argv[1];
    std::string model_path = (argc > 2) ? argv[2] : "../01-models/yolov5su_fp32.onnx";
    std::string device = (argc > 3) ? argv[3] : "CPU";

    std::cout << "========================================" << std::endl;
    std::cout << "YOLOv5s C++ OpenVINO Inference Pipeline" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    OVInference infer(model_path, device);
    ...
    std::cout << "[MAIN] Running inference (C++ via OpenVINO)..." << std::endl;
